...
$cat = "ourProducts/corporateGifts";
$subcat= "";
...
foreach($subcatlist as $subcat){
	...
	elseif($subcat == 'Bags And Wallets'){
		
		
		$tempcatlist= scandir($cat."/".$subcat);
		
		
		
		foreach($tempcatlist as $tempcat){
			
			if($tempcat=="." || $tempcat == ".."){
			continue;
			}
			$modelslist = scandir($cat."/".$subcat.'/'.$tempcat);
		
		echo('<article  id="'.$tempcat.'style="margin-top: 0px;">

	<h3 style="padding: 13px; background-color: #d11237; color: #fff;" align="center">'.$tempcat.'</h3>
	...
	<div class="row">');
		...
		foreach($modelslist as $model){
			...
			if($model == "." || $model == ".."){
				...
				<img class="card-img-top" src="'.$cat.'/'.$subcat.'/'.$tempcat.'/'.$model.'/'.$model.'.jpg" alt="Card image cap" width="100%"  >
